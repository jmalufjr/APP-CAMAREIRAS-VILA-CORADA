...
create table room_bills (
  ...
  -- Incrementada a cada mudança nesta conta OU em qualquer item/comanda
  -- dela (via trigger — ver bump_room_bill_version/bump_parent_bill_*
  -- mais abaixo), junto com updated_at. Base do histórico de mudanças
  -- versionado (room_bill_change_events) usado pela API de consumos.
...
-- ============================================================================
-- API DE CONSUMOS (PRD_consumos-api-joao-v1.md) — tokens de serviço e
-- histórico de mudanças versionado
-- ============================================================================

-- Tokens de acesso de serviço pra API externa de consumos (item 5) — nunca
-- guardamos o token em texto puro, só o hash SHA-256; o valor real só é
-- mostrado uma vez, no momento da criação, na tela do admin.
...
-- Histórico de mudanças versionado (item 4 — endpoint /changes).
-- Implementado via trigger no banco, não em código TypeScript: garante que
-- NENHUM ponto de escrita (Server Action, função SQL security definer,
-- sincronização com a Stays) pode esquecer de registrar uma mudança,
-- porque nenhum deles precisa saber que isso existe.
