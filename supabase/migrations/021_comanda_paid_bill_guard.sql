-- Bug encontrado em revisão de código (Parte 05): a correção da migration
-- 019 bloqueou editar/cancelar uma comanda cuja conta está 'fechada', mas
-- esqueceu de bloquear também quando a conta já está 'paga'. Como uma
-- camareira pode navegar diretamente para /comanda/<id>/editar de qualquer
-- comanda (inclusive uma antiga, de uma conta já paga, cujo quarto hoje já
-- tem uma conta nova 'aberta' em andamento), isso permitia reabrir e
-- alterar/cancelar uma comanda que já fazia parte de uma conta encerrada e
-- paga — corrompendo o histórico financeiro daquele pagamento e, no caso de
-- edição, somando indevidamente os itens numa conta de OUTRO quarto.
